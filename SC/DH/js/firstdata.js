...
function hcontent(data) {
    ...
    conhtml = '<article class="conWrap" style="width: ' + conWrapWidth + 'px;overflow: hidden">' +
        '<article class="pNums"><img src="img/people.png" class="leftimg">' +
        '<ul><li search="BRZS-病人总数" onclick="showCount(this)"><span>' + XSNRdata[0] + '</span><span class="totoal pNum">' + RSdata[0] + '</span></li>' +
        '<li class="spanbtn" search="RY-入院" onclick="showLists(this)"><span>' + XSNRdata[1] + '</span><span class="innum pNum">' + RSdata[1] + '</span></li>' +
        '</ul>' +
        ...
        '<li class="spanbtn" search="SW-死亡-all" onclick="showbeds(this)"><span>' + XSNRdata[4] + '</span><span class="deathnum pNum">' + RSdata[4] + '</span></li>' +
        '</ul></article><article class="tinOut"><img src="img/post.png" class="middleimg">' +
        '<h2>今日出院</h2>' +
        '<ul><li class="spanbtn" search="JRCYSW-今日出院（上午）-all" onclick="showbeds(this)"><span>上午</span><span class="ttout pNum">' + RSdata[5] + '</span></li>' +
        ...
        '<li class="spanbtn" search="MRCYXW-明日出院（下午）-all" onclick="showbeds(this)"><span>下午</span><span class="ybout pNum">' + RSdata[8] + '</span></li>' +
        '</ul>' +
        '</article>' +
        '<article class="operation">' +
        '<img src="img/operation.png"76iutr , class="rightimg">' +
        '<ul style="float: left">' +
        '<li class="spanbtn" search="8-今日手术-all" onclick="showbeds(this)"><span>' + XSNRdata[9] + '</span><span class="toper pNum">' + RSdata[9] + '</span></li>' +
        '<li class="spanbtn" search="9-明日手术-all" onclick="showbeds(this)"><span>' + XSNRdata[10] + '</span><span class="yoper pNum">' + RSdata[10] + '</span></li>' +
        '</ul>' +
        '<ul style="float: left;margin-left: 20px">' +
        '<li class="spanbtn" search="10-昨日手术-all" onclick="showbeds(this)"><span>' + XSNRdata[11] + '</span><span class="toper pNum">' + RSdata[9] + '</span></li>' +
        '</ul>' +
        '</article>' +
        '<article class="xtyds"><img src="img/dtxt.png" class="lastimg">' +
        '<ul style="width: ' + xtydsulWidth + 'px">' + xtydsHTML + '</ul>' +
        '</article></article><span class="shadowline"></span>';
...
function pagedata(data, grouplist) {
    ...
    console.log(listLen, 'listLenlistLenlistLen')
    ...
    for (var i = 0; i < listLen; i++) {
        ...
        for (var j = 0; j < cglen; j++) {
            ...
            if (j == cglen - 1) {
                cghlcont += '<li class="' + liclass + '" style="padding-right:22px"><span class="' + btnclass + '" search="' + RWXH + '-' + RWName + '" onclick="showbeds(this)">' + RWName + '</span>' + mark + '</li>';
            } else {
                cghlcont += '<li class="' + liclass + '"><span class="' + btnclass + '" search="' + RWXH + '-' + RWName + '" onclick="showbeds(this)">' + RWName + '、' + '</span>' + mark + '</li>';
            }
        }
        ...
        for (var k = 0; k < zklen; k++) {
            ...
            if (k == zklen - 1) {
                zkhlcont +=
                    ...
                    '<span class="spanbtn" search="' + RWXH + '-' + RWName + '" onclick="showbeds(this)">' + RWName + '</span>' +
                    '</li>';
            } else {
                zkhlcont +=
                    '<li>' +
        ...
        pagehtml += '<li>' +
            '<ul class="coninfos">' +
            '<li class="newmark">' + newmark + '</li>' +
            '<li class="bednum"><span class="' + numstyle + '">' + bednum + '</span></li>' +
            '<li  class="ageimg" pos = "img' + grouplist + i + '">' + imgUrl + '</li>' +
            '<li class="nm-care"><article id="nm_bd' + grouplist + i + '" class="xMarquee nmmar">' +
            '<ul>' + cghlcont + '</ul>' +
            '</article>' +
            '</li>' +
            '<li class="sp-care">' +
            '<article id="sp_bd' + grouplist + i + '" class="xMarquee spmar">' +
            '<ul>' + zkhlcont + '</ul>' +
            '</article>' +
            '</li>' +
            '<li class="status"><article id="stu_bd' + grouplist + i + '" class="xMarquee stumar">' +
            '<ul>' + status + iconArr[i] + '</ul>' +
            '</article>' +
            '</li>' +
            '</ul>' +
            '</li>';
    ...
    var datafooter = '';
    datafooter = ' <article class="groupwrap"><div class="grouptext">责任制分组</div><div class="groupbg">' + groupnum + '</div></article><article data-num="' + packetnum + '" class="dnwrap no-padding"><div class="dcwrap"><div class="dataimgbox"><div class="dataimg"><img src="img/' + dcname + '.png" alt="医生头像"></div></div><div class="pull-left datadcbox"><input id="dcname" class="dcname" type="text" value="' + dcname + '"/><input id="dcjob" class="dcposition" type="text" value="' + dcposition + '"><div class="dctitle">责任医生</div><div class="dcline"></div></div></div><div class="nswrap"><div class="dataimgbox"><div class="dataimg"><img src="img/' + nurse + '.png" alt="护士头像"></div></div><div class="pull-left datadcbox"><input id="nsname" class="dcname" type="text" value="' + nurse + '"/><input id="nsjob" class="dcposition" type="text" value="' + nposition + '"><div class="dctitle">责任护士</div><div class="dcline"></div></div></div></article><article class="iconswrap"><div class="legendline"><span class="spanbtn" search="46-跌倒评估-4" onclick="showbeds(this)"><img src="img/fall.png" class="ficon fallicon"></span><span class="legendtext">跌倒评估</span><span class="spanbtn" search="47-压疮评估-3" onclick="showbeds(this)"><img src="img/ass.png" class="ficon assicon"></span><span class="legendtext">压疮评估</span><span class="spanbtn" search="48-疼痛评估" onclick="showbeds(this)"><img src="img/hurt.png" class="ficon hurticon"></span><span class="legendtext">疼痛评估</span><span class="spanbtn" search="69-关注病人" onclick="showbeds(this)"><img src="img/care.png" class="ficon careicon"></span><span class="legendtext">关注病人</span></div><div class="legendline"><span class="spanbtn" search="19-病情保密" onclick="showbeds(this)"><img src="img/sec.png" class="ficon secicon"></span><span class="legendtext">病情保密</span><span class="spanbtn" search="45-导管评估-5" onclick="showbeds(this)"><img src="img/duct.png" class="ficon ducticon"></span><span class="legendtext">导管评估</span><span class="spanbtn" search="20-床边隔离" onclick="showbeds(this)"><img src="img/bed.png" class="ficon bedicon"></span><span class="legendtext">床边隔离</span><span class="spanbtn"><img src="img/danger.png" class="ficon dangericon"></span><span class="legendtext">危急值</span></div></article>';

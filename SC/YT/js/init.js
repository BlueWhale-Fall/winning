function initBQDM(data) {
    ...
    initimg.addEventListener('click', function (e) {
    ...
    $('.zoomlist li').click(function () {
        ...
        $('#initwrap footer .hosname').eq(0).html(data.BQInfos[_index].HOSName);
        localStorage.setItem('currentZoom', JSON.stringify(data.BQInfos[_index]));
        $('.listwrap').eq(0).slideUp();
    });

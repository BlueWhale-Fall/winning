var area = {
    ...
    renderContent: function () {
        ...
        for (var i = 0; i < contentData.length; i++) {
            dom_string += '<li class="' + this.getBackgroundColor(contentData[i].NAME, contentData[i].GRADE) + '">' +
                '            <div class="content-header clear">' +
                '                <div class="float-left">' + contentData[i].CH + '</div>' +
                '                <div class="float-right">' + contentData[i].NAME + '</div>' +
                '            </div>' +
                '            <div class="content-body">' +
                '                <img src="img/' + this.getImgType(contentData[i].NAME, contentData[i].SEX) + '.png" alt="">' +
                '                <ul style="right: 20px;top: 8px;">';
            if (contentData[i].NAME) {
                dom_string += '<li>年龄：<span>' + contentData[i].AGE + '</span></li>'
                var DJ=contentData[i].WZZT.charAt(1)
                if (DJ==='重'||DJ==='危'){
                    console.log(DJ);
    ...
    getBackgroundColor: function (name, grade) {
        if (name) {
            if (grade == 'I') {
